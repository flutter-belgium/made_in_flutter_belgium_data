...
Future<void> _setLinkedProjectsToCompanies(
  ...
  for (final company in companies) {
    ...
    company.projects =
        projectsForCompany.map((e) => e.toMinimizedProject()).toList();
    company.involvedProjects =
        involvedProjectsForCompany.map((e) => e.toMinimizedProject()).toList();
  }
}

Future<void> _setInvolvedCompaniesToProjects(
    List<Project> projects, List<Company> companies) async {
  for (final project in projects) {
    ...
    for (final developer in developers) {
      ...
      final company =
          companies.firstWhereOrNull((e) => e.name == companyNameForDeveloper);
      if (company == null) continue;
      involvedCompanies.add(company);
    }
    project.involvedCompanies =
        involvedCompanies.map((e) => e.toMinimizedCompany()).toList();
  }
}
...
Future<void> _setLinkedProjectsToDevelopers(
  ...
  for (final developer in developers) {
    ...
    final projectsForDeveloper = projects.where((project) {
      ...
      return projectDevelopers.any((projectDeveloper) =>
          projectDeveloper.githubUserName == developer.githubUserName);
    }).toList();
    developer.projects =
        projectsForDeveloper.map((e) => e.toMinimizedProject()).toList();
  }
}
